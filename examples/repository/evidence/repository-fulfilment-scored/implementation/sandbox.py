...
class DockerSandbox:
    ...
    def run(self, source: str, entrypoint: str, cases: list[dict]) -> TestResult:
        ...
        execution = self.execute(DRIVER, payload)
        output, status = execution["stdout"], execution["status"]
        returncode = execution["returncode"]
        try:
            rows = json.loads(output)["results"] if status == "completed" and returncode == 0 else []
        ...
        return TestResult(sum(c["passed"] for c in checked), len(cases), checked, status, execution["seconds"], image)

    def execute(self, driver: str, payload: bytes) -> dict:
        """Run a trusted protocol driver with untrusted input in the same isolation."""
        if len(payload) > 2_000_000:
            raise ValueError("Sandbox input exceeds 2 MB")
        image = self.inspect()["image"]
        name = "dream-rsi-" + uuid.uuid4().hex
        started = time.monotonic()
        output, errors = bytearray(), bytearray()
        overflow = threading.Event()
        command = self.command(name, image)
        command[-1] = driver
        ...
        return {"stdout": bytes(output), "stderr": bytes(errors), "returncode": process.returncode,
                "status": status, "seconds": time.monotonic()-started, "image": image}
